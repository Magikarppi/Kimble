...
const findPlayerAndPieceAtPosition = (position: number) => {
...
while (!gameOver) {
    ...
    if (player.positions[activePieceIndex] === basePosition) {
        ...
        } = findPlayerAndPieceAtPosition(newPositionValue);
        ...
        if (
            ...
            console.log(
                `Player ${currentPlayerIndex + 1} cannot move piece ${
                    activePieceIndex + 1
                } to position ${newPositionValue} because that would "go over the board".`
            );
        } else {
